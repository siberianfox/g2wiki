...
- A common mistake is to not update the motate submodule. Whenever you pull from github be sure to run `git submodule update` from the command line (e.g. OSX Terminal) to make sure the right commit of Motate is associated with the g2core you have.
...
- When firing up a new instance of AS7 you may get an error trying to connect to the ICE. Disconnect / reconnect the ICE - in Parallels: /Devices/External Devices, Atmel-ICE...  Disconnect and reconnect.
