...
- In fact, it's good practice to just delete the /bin and /build directories when you pull a new branch, as the `clean` command can fail on Windows due to permissions problems. If your build behaves badly (goes into panic, does completely unexpected things) it's a good idea to delete these directories and recompile.
