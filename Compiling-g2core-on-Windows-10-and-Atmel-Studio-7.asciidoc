...
** Problems may be because of a low-quality USB hub in the chain (we've seen this a lot). This can cause all kinds of other connection problems. Replace the hub with a high-quality one, and if on a Macbook with USB B connection use the USB on the right side of the computer (one less hub in the chain).
