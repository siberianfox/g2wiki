...
NOTE: Windows 10 and VMware are like an old married couple. She (W10) has had a face lift but that did not change her  underlying nature, and he's 50 lb overweight and refuses to go on a diet. But they still live together and continue to argue.
...
*** Install the 2 ARM architectures (32 bit ARM, Smart Arm) but not the AVR8 bit architecture (unless you are doing 8 bit development like the TinyG project separately from g2core). This is required even though the g2core project downloads its complete toolchain.

*** We do not install the Atmel Studio Framework code or examples. You do NOT want this.
...
NOTE: If the g2core project is present but the Motate project is missing when you fire up AS& check out this troubleshooting item: https://github.com/synthetos/g2/wiki/Troubleshooting#problem-the-motate-directory-is-empty-the-motate-project-is-missing 
