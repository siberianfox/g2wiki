...
*** Do this by clicking the `g2core.atsln` file in the g2 directory. This should bring up bot the g2core and Motate projects in AS7. Yes, you trust it, when asked. If for some reason the Motate project does not load it may because you didn't initialize the Motate directory using `git submodule update --init`
