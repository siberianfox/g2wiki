...
** Make sure the ICE is selected in the `g2core` window, `Tools` menu
** Go to `Tools`, `Device Programming` and hit Apply. SOMetimes it takes more than one try
** Disconnect / reconnect the ICE - in Parallels: /Devices/External Devices, Atmel-ICE...  Disconnect it, wait a second, then reconnect it.
