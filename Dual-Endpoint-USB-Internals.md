...
(Title) An explanation of what's going on with these device classes, virtual functions and inheritance. For those that do not know C++ or need a refresher.
...
That says we have a template structure named xioDeviceWrapper that is a subclass (child) of xioDeviceWrapper_t and has the following definition (in {…};
…actually xioDeviceWrapperBase, not _t
like a normal structure). This means that whatever is defined in xioDeviceWrapperBase will also implicitly be defined in xioDeviceWrapper.
...
I’m out of time and this email is too long already.
